...
export const UserCard = ({ styles, user, isBanned, onOpen }: UserCardProps) => {
    ...
    <button type="button" className={styles.userCard} onClick={onOpen}>
      ...
      <div className={styles.cardStats}>
        <span>
          ...
          <small>{t("users.card.timeBank")}</small>
        </span>
        <span>
          <strong>{formatAdminDate(user.registeredAt)}</strong>
          <small>{t("users.card.joined")}</small>
        </span>
      </div>

      <div className={styles.cardFooter}>
        <span>{user.currentLevelCode || t("users.card.noLevel")}</span>
        <span className={styles.openLink}>
          {t("users.card.summary")}
