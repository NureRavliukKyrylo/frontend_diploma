...
export const DrawerRecentRequestsSection = ({
  ...
  return (
    <div className={styles.drawerSection}>
      <div className={styles.drawerSectionTitle}>Recent requests</div>
      {requests.length ? (
        <div className={styles.requestList}>
          {requests.map((item) => (
            <RequestRow key={item.requestId} styles={styles} item={item} t={t} />
          ))}
        </div>
      ) : (
        <div className={styles.emptyRequests}>
          <Gift size={20} aria-hidden="true" />
          <span>No recent requests for this user.</span>
        </div>
      )}
    </div>
  );
